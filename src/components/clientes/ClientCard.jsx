...
export default function ClientCard({ r, onEdit, onArchive, onDelete, canDelete, profile }) {
  ...
  const mensajeWa = encodeURIComponent(
    `Hola ${r?.nombres || "Cliente"}, te saluda ${nombreAsesor} de Banco Caja Social. Como tu banco amigo, te recordamos que tenemos programada nuestra visita de seguimiento para el día de mañana. ¿Te queda bien el horario acordado para reunirnos? ¡Un amigo hoy, mañana y siempre!`
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", top: -8, right: 14 }}>
        <Stamp estado={r?.categoria_cliente || r?.estado || "Nuevo"} />
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14, borderTop: `1px solid ${C.line}`, paddingTop: 12 }}>
        ...
        <IconBtn icon={MessageCircle} label="WhatsApp (Recordatorio)" href={wa} disabled={!r?.whatsapp && !r?.telefono} />
