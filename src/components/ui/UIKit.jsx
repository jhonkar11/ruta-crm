...
import { C, ESTADO_STYLE, CITA_ESTADO_STYLE, inputStyle, iconRow } from "../../styles/tokens";

export function Stamp({ estado, size = "md", rotate = true, kind = "cliente" }) {
  const map = kind === "cita" ? CITA_ESTADO_STYLE : ESTADO_STYLE;
  const s = map[estado] || map["Pendiente"] || Object.values(map)[0];
  ...
  return (
    <span
      style={{
        ...
        fontWeight: 700, fontSize: font, letterSpacing: "0.06em", padding: pad,
        borderRadius: 4, border: `1.5px solid ${s.fg}22`, textTransform: "uppercase",
        transform: rotate ? "rotate(-2.5deg)" : "none", display: "inline-block",
        boxShadow: "0 1px 0 rgba(0,0,0,0.08)", whiteSpace: "nowrap",
      }}
    >
      {estado}
    </span>
